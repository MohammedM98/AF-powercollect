<x-app-layout>
    ...
    @if (session('status') === 'branch-created')
        <div class="mb-4 text-sm font-medium text-green-600">{{ __('Branch created.') }}</div>
    @elseif (session('status') === 'branch-updated')
        <div class="mb-4 text-sm font-medium text-green-600">{{ __('Branch updated.') }}</div>
    @endif
                ...
                @foreach ($branches as $branch)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-end">
                            <a href="{{ route('branches.edit', $branch) }}" class="font-medium text-brand-600 hover:underline">{{ __('Edit') }}</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
