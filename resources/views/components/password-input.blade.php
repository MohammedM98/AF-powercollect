...
<div class="relative" x-data="{ show: false }">
    <input :type="show ? 'text' : 'password'" @disabled($disabled) {{ $attributes->merge(['class' => 'border-gray-300 focus:border-brand-500 focus:ring-brand-500 rounded-md shadow-sm pe-10']) }}>
    <button
        type="button"
        tabindex="-1"
        @click="show = !show"
        :aria-label="show ? '{{ __('Hide password') }}' : '{{ __('Show password') }}'"
        class="absolute inset-y-0 end-0 flex items-center px-3 text-gray-400 transition hover:text-gray-600"
    >
