...
/**
 * The first cell of a row: a graphite tile (an icon, or the name's first
 * letter), the name, and a quiet line under it. `status` adds a small dot
    ...
    return (
        <div className="flex items-center gap-3">
            <span className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-[13px] bg-graphite-gradient font-display dark:ring-1 dark:ring-white/10 text-[13px] font-bold text-white">
                {icon ? <Icon name={icon} className="h-[18px] w-[18px]" /> : (name ?? '').trim().substring(0, 1)}
                {status && (
